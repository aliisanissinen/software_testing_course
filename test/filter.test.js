...
describe('Testing filter function', function () {
  ...
  it('2. Predicate truthy for all array elements', function (done) {
    ...
    for (const i in sizes) {
      ...
      expect(filter(array, hasOpenOrders))
        .to.be.a('Array')
        .to.eql(originalArray)
  ...
  it('4. Predicate truthy for some array elements', function (done) {
    ...
    for (let i = 0; i < l; i++) {
      const originalProducts1 = allFalse.slice()
      originalProducts1[i] = savedProducts[0]
      const products1 = originalProducts1.slice()
      ...
      for (let j = i + 1; j < l; j++) {
        const originalProducts2 = originalProducts1.slice()
        originalProducts2[j] = savedProducts[0]
        const products2 = originalProducts2.slice()
        ...
        for (let k = j + 1; k < l; k++) {
          const originalProducts3 = originalProducts2.slice()
          originalProducts3[k] = savedProducts[0]
          const products3 = originalProducts3.slice()
          expect(filter(products3, hasOpenOrders))
            .to.be.a('Array')
            .to.eql(true3)
          expect(products3).to.eql(
            originalProducts3,
            iteratedArrayChangedError
          )
        }
      }
    }
